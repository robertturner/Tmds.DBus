...
namespace Tmds.DBus.Protocol
{
    static internal class ArgTypeInspector
    {
        ...
        private static readonly Type s_ireadonlydictionaryGenericType = typeof(IReadOnlyDictionary<,>);
        ...
        public static bool IsDBusObjectType(Type type)
        {
        ...
        public static EnumerableType InspectEnumerableType(Type type, out Type elementType)
        {
            elementType = null;
            var typeInfo = type.GetTypeInfo();
            ...
            if (typeInfo.IsArray)
            {
                ...
                return InspectElementType(typeInfo.GetElementType());
            }
            if (typeInfo.GetCustomAttribute<DictionaryAttribute>(false) != null)
            {
            ...
            if (typeInfo.IsGenericType &&
                (typeInfo.IsInterface || typeInfo.GetGenericTypeDefinition() == typeof(Dictionary<,>)))
            {
                var genericTypeDefinition = typeInfo.GetGenericTypeDefinition();
                bool good = typeInfo.IsInterface && (genericTypeDefinition == s_idictionaryGenericType || genericTypeDefinition == s_ireadonlydictionaryGenericType);
                if (!good)
                {
                    var ifces = typeInfo.GetInterfaces();
                    //if (s_idictionaryGenericType.IsAssignableFrom(typeInfo) || typeInfo.IsAssignableFrom(s_idictionaryGenericType))
                    good = ifces.Where(i => i.IsGenericType).Any(i =>
                    {
                        var genDef = i.GetGenericTypeDefinition();
                        return genDef == s_idictionaryGenericType || genDef == s_ireadonlydictionaryGenericType;
                    });
                }
                if (good)
                {
                    ...
                    return InspectElementType(typeInfo.GenericTypeArguments[0]);
                else
                    return EnumerableType.NotEnumerable;
            }
        ...
        private static EnumerableType InspectElementType(Type elementType)
        {
            ...
            return isKeyValuePair ? EnumerableType.EnumerableKeyValuePair : EnumerableType.Enumerable;
        }
